...
- `v1.0.4` page: https://github.com/bwjbuild/Vid2Script/releases/tag/v1.0.4
- Direct EXE (`v1.0.4`): https://github.com/bwjbuild/Vid2Script/releases/download/v1.0.4/Vid2Script.exe
- SHA256: generated by GitHub Actions build (see release artifacts)

## Changelog

### v1.0.4 (2026-06-12)

- **Drop Android player client** — YouTube has been aggressively blocking the Android client in 2026. Switched to `web` + `ios` only.
- **Fix cookie retry logic** — Cookie retry no longer fires for every YouTube error. Only actual auth challenges trigger browser-cookie fallback, so real errors (extractor failures, network issues) are reported immediately instead of being masked by a 2-minute cookie retry loop.
- **Modernized format selection** — `bestaudio*` replaces the old `bestaudio/best` syntax.
- **Added extractor retries** — `extractor_retries: 3` and `file_access_retries: 3` to handle transient YouTube failures more gracefully.
- **Robust output detection** — Replaced fragile `prepare_filename()` guess with a before/after file snapshot so the tool reliably finds the converted MP3 regardless of yt-dlp's internal naming.
